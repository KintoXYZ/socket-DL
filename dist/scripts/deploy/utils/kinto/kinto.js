...
exports.estimateGas = exports.whitelistApp = exports.deployOnKinto = exports.handleOps = exports.isKinto = void 0;
...
const deployWithDeployer = async (contractName, argTypes, args, signer) => {
    ...
    try {
        const owner = await (await getInstance(contractName, contractAddr, signer)).owner();
...
const handleOps = async (userOps, signer, gasParams = {}, withPaymaster = false) => {
    ...
    const txResponse = await entryPoint.handleOps(userOps, await signer.getAddress(), gasParams);
...
const getInstance = async (contractName, address, signer) => (await hardhat_1.ethers.getContractFactory(contractName))
    .attach(address)
    .connect(signer);
